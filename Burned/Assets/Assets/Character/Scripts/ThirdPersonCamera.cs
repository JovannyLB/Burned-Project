...
//            }
            ...
            if (Physics.Linecast(target.transform.position, target.position - transform.forward * distanceFromTarget, out hit) && !target.transform.root.GetComponent<PlayerController>().aiming){
                ...
                if (!target.transform.root.GetComponent<PlayerController>().aiming){
                    DOTween.To(() => actualDistanceFromTarget, x => actualDistanceFromTarget = x, distanceFromTarget, 0.1f).OnComplete(() => { actualDistanceFromTarget = distanceFromTarget; });
                }
                else{
                    DOTween.To(() => actualDistanceFromTarget, x => actualDistanceFromTarget = x, distanceFromTargetAim, 0.1f).OnComplete(() => { actualDistanceFromTarget = distanceFromTargetAim; });
                }
            }
